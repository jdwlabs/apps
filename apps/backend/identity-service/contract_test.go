...
const contractPath = "../usersrole/docs/contracts/identity-service.openapi.yaml"
...
func TestTheContractStillDescribesEighteenAuthorizedOperations(t *testing.T) {
	...
	for _, operation := range operations {
		switch {
		case strings.HasPrefix(operation.path, "/auth/"):
			surface["auth"]++
		case strings.HasPrefix(operation.path, "/api/users"):
			surface["users"]++
		case strings.HasPrefix(operation.path, "/api/roles"):
			surface["roles"]++
		default:
			t.Errorf("%s is on none of the three surfaces this service owns", operation.key())
		}
		if operation.rule == "" {
			t.Errorf("%s names no authorization rule", operation.key())
		}
	}
	for surfaceName, want := range map[string]int{"auth": 2, "users": 8, "roles": 8} {
		if surface[surfaceName] != want {
			t.Errorf("%s carries %d operations, want %d", surfaceName, surface[surfaceName], want)
		}
	}
}

func TestNothingOutsideAuthIsFrozenAsPublic(t *testing.T) {
	// The filter chain's permitAll matchers are what separate PUBLIC from
	// AUTHENTICATED, and neither carries a predicate. A path that drifted onto
	// the public list would be reachable with no token and nothing else in the
	// build would notice.
	for _, operation := range contractOperations(t) {
		if operation.rule == authz.RulePublic && !strings.HasPrefix(operation.path, "/auth/") {
			t.Errorf("%s is frozen as PUBLIC outside /auth", operation.key())
		}
	}
}
...
func TestEveryRuleTheContractNamesHasAPrincipalThatMustPassIt(t *testing.T) {
	for _, operation := range contractOperations(t) {
...
func TestEveryRuleThatDiscriminatesHasAPrincipalThatMustFailIt(t *testing.T) {
	// A rule with no deny case would pass its allow cases with the check removed
	// altogether. Two rules genuinely refuse no verified principal; they are
	// named rather than inferred, so a third cannot join them silently.
	for _, operation := range contractOperations(t) {
		if undiscriminatingRules[operation.rule] {
			continue
		}
	...
	for rule := range undiscriminatingRules {
		if len(deniedBy(rule)) != 0 {
			t.Errorf("rule %s is listed as refusing nobody and yet names a principal it refuses", rule)
		}
	}
}
...
func TestTheContractsOperationIdsAreTheOnesTheJvmControllersDeclare(t *testing.T) {
	...
	want := []string{
		"authenticate", "createRole", "createUser", "deleteRole", "deleteUser",
		"getAllRoles", "getAllUsers", "getRoleById", "getRoleByName", "getUserByEmailAddress",
		"getUserById", "grantRolesToUser", "grantUsersToRole", "registerUser",
		"revokeRolesFromUser", "revokeUsersFromRole", "updateRole", "updateUser",
	}
...
func TestTheParityCasesNameTheOperationsTheContractIdentifies(t *testing.T) {
	// The parity case's own name is what a failure is reported under, so it has
	// to be the contract's operationId rather than a paraphrase of it.
	identified := map[string]string{}
	for _, operation := range contractOperations(t) {
		identified[operation.key()] = operation.operationID
	}

	for _, tc := range parityCases() {
		key := tc.method + " " + contractShape(tc.path)
		if want := identified[key]; tc.operation != want {
			t.Errorf("the case for %s is named %q, the contract calls it %q", key, tc.operation, want)
		}
	}
}
