...
func TestGitHubOpenPRRejectsRepo(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			_, err := NewGitHubClient(srv.URL, StaticGitHubToken("ghtok"), tc.allowed, testPathGlobs, nil, srv.Client()).OpenPR(context.Background(), p, "JDWLABS-500")
			if err == nil {
...
func TestGitHubRepoRefusalNamesBothSides(t *testing.T) {
	g := NewGitHubClient("http://unused", StaticGitHubToken("ghtok"), []string{"jdwlabs/platform", "jdwlabs/deployments"}, testPathGlobs, nil, nil)
...
func TestGitHubEmptyAllowlistIsNotARepoRejection(t *testing.T) {
	err := NewGitHubClient("http://unused", StaticGitHubToken("ghtok"), nil, testPathGlobs, nil, nil).checkRepo("jdwlabs/platform")
	if err == nil {
