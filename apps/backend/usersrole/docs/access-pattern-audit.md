...
The four indexes have since been added: `profiles_user_id_idx`,
`addresses_profile_id_idx`, `profile_icons_profile_id_idx` and
`users_roles_role_id_idx`, in the `V3__auth_fk_indexes` Flyway migration that the
`jdwlabs-schemas` service applies to both clusters, and in `00_schema.sql` for
fresh installs. The inventory above is the pre-migration measurement and is left
as it was taken. The planner still chooses a sequential scan for every one of
these predicates, because each table fits a single page at current row counts;
the index is what stops that being true as the tables grow. The N+1 shape is
untouched and still belongs to the Go data-access design.
...
| 3   | ~~**The bcrypt password hash is returned by every `User` response**, including the public `POST /auth/user`.~~ Closed.                                                                        | High   | Done: `User.password` is `@JsonIgnore`, the request DTO's password is write-only, and a guard test scans every model and DTO for password-like fields left serializable.                             |
| 4   | ~~**Four foreign-key columns on the hydration path are unindexed** (section 3.7). Invisible at 9 rows, quadratic as data grows.~~ Closed.                                                     | High   | Done: the `V3__auth_fk_indexes` Flyway migration in the `jdwlabs-schemas` service indexes all four columns, and `00_schema.sql` carries them for fresh installs.                                     |
...
| 6   | ~~**`GET /api/roles` is unpaginated** and its fan-out is `1 + N` over the whole table.~~ Closed.                                                                                              | Medium | Done: the frozen identity contract gives it `page` and `size` over a stable `role_id` ordering, recorded as a behaviour change with its client impact analysed.                                      |
| 7   | ~~**Any authenticated principal can enumerate all roles** (section 2.3).~~ Decided.                                                                                                           | Medium | Decided: frozen as-is, because no client-side role gate exists and tightening would 403 the role screens at cutover. The residual exposure — the `users` array on each role — is named as follow-up. |
...
| 9   | ~~**The icon DAO's identifier is inconsistent** between `create`, `findById`, `update` and `deleteById` (section 3.6).~~ Closed.                                                              | Medium | Done: the frozen profile contract makes `profile_id` the icon's only identifier. `ProfileIcon.id` stays in the payload as an opaque surrogate no route accepts and no lookup uses.                   |
| 10  | ~~**The `profile_id` claim is minted at login and never refreshed.**~~ Decided.                                                                                                               | Medium | Decided: `profile-service` falls back to a `user_id`-keyed lookup when the claim is absent. No re-minting, no synchronous call back into identity.                                                   |
| 11  | ~~**Deleting a user cascades across the boundary**~~ Decided.                                                                                                                                 | Medium | Decided: one local transaction in `identity-service` over `ON DELETE CASCADE`, rather than a cross-service call. The coupling this costs `profile-service` is recorded in the contract.              |
...
The five decisions above were taken in the contract work and are written up in
`docs/contracts/README.md`, alongside the two the audit had already fixed — the
amended `User` payload without the embedded profile, and the write-only password.
Each also appears as an `x-` extension on the operation or schema it governs in
`docs/contracts/identity-service.openapi.yaml` and
`docs/contracts/profile-service.openapi.yaml`.
