...
(feat=minor, fix=patch, breaking=major; chore/docs/etc never bump).

**The git tag is the only record of a version.** No manifest in the tree carries one,
`nx release` runs with `git.commit: false`, and a release pushes tags and nothing else —
so it never writes to `main`. Delivery targets resolve the version they ship with
`scripts/resolve-version.sh` (custom manifest-free version actions in `tools/release/`).
GitHub Releases are the changelog; no `CHANGELOG.md` is generated. A brand-new project
needs an initial tag before its first release. See [docs/workflows.md](docs/workflows.md).
**Never run `nx release` locally without `--dry-run`.**
...
5. **Main push only:** the `release` job resolves every project's delivery targets from `nx graph` _first_, then runs `nx release` (version, tag, GitHub Release) and joins the pushed tags against that map. Downstream: per-project deliver matrix (Docker image → deployments Helm bump → Docker Hub description), then E2E dispatch.
